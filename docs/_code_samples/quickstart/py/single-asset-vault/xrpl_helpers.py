...
def create_single_asset_vault(vault_owner_seed, asset_maximum, is_transferable=True):
    ...
    try:
        response = submit_and_wait(
            tx, client=client, wallet=owner_wallet, autofill=True)
        return tx.to_dict, response.result
    except XRPLRequestFailureException as e:
        return tx.to_dict(), f"Error: {e.error} - {e.error_message}"


def update_single_asset_vault(vault_owner_seed, asset_maximum):
    """Update settings of an existing vault on the XRPL"""
    client = get_client()
    owner_wallet = Wallet.from_seed(vault_owner_seed)
    set_flags = []
    tx = VaultSet(
...
def convert_drops_to_xrp(drops_amount):
    return float(drops_to_xrp(drops_amount))
