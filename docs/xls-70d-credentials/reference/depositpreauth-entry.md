...
A `DepositPreauth` entry tracks a preauthorization from one account. Anyone can send [DepositPreauth transactions][] to create these entries, but they have no effect unless you are using [Deposit Authorization](https://xrpl.org/docs/concepts/accounts/depositauth).
...
| `AuthorizeCredentials` | String | Array   | No | The set of credentials that received preauthorization. (Any account with these credentials is preauthorized.) This array has a minimum length of 1 and a maximum length of 8 credentials. |
...
<!-- common link definitions. When migrating to xrpl.org, remove these and use the definitions in the common-links snippet. -->
...
[common fields]: https://xrpl.org/docs/references/protocol/ledger-data/common-fields/
