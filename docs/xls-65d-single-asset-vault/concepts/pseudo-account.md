...
A pseudo-account is a special type of account with its own `AccountRoot` ledger entry, but only holds assets on behalf of an on-chain protocol. Use cases for pseudo-accounts include:
