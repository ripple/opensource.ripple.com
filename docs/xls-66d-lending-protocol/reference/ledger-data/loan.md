...
A `Loan` ledger entry defines the state of an on-chain loan agreement between a _Loan Broker_ and a _Borrower_. It contains all the details of the loan, such as fees and interest rate. You can create a `Loan` object with the [`LoanSet`](../transactions/loanset.md) transaction.
...
| `Sequence`            | Number    | UInt32        | Yes       | The transaction sequence number that created the loan. |
...
| `ClosingInterestRate` | Number    | UInt16        | Yes        | The interest rate charged for repaying the loan early in 1/10th basis points. Valid values are from `0` up to `100000` (0% - 100%). |
